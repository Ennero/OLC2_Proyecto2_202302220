...
static void emitir_array_init_rec(AbstractExpresion *init_node, TipoDato base_tipo, int depth, FILE *ftext)
{
    if (!init_node || strcmp(init_node->node_type ? init_node->node_type : "", "ArrayInitializer") != 0)
    {
        ...
        if (base_tipo == STRING)
        {
            if (!emitir_eval_string_ptr(init_node, ftext))
    ...
    if (depth <= 1)
    {
        ...
        {
            char mvm[64];
            snprintf(mvm, sizeof(mvm), "    mov w1, #%d", m);
            emitln(ftext, mvm);
        }
        emitln(ftext, "    str w1, [sp]");
        emitln(ftext, "    mov w0, #1");
        ...
        // Llamar a la función de creación de arreglos según el tipo base
        if (base_tipo == STRING || base_tipo == DOUBLE || base_tipo == FLOAT)
            emitln(ftext, "    bl new_array_flat_ptr");
        else
            emitln(ftext, "    bl new_array_flat");
        ...
        for (int j = 0; j < m; ++j)
        {
            {
                char movj[64];
                snprintf(movj, sizeof(movj), "    mov x23, #%d", j);
                emitln(ftext, movj);
            }
            if (base_tipo == STRING)
            {
                if (!emitir_eval_string_ptr(lista->hijos[j], ftext))
                    emitln(ftext, "    mov x1, #0");
                emitln(ftext, "    str x1, [x22, x23, lsl #3]");
            }
            else if (base_tipo == DOUBLE || base_tipo == FLOAT)
    ...
    // Construir arreglo de punteros (8 bytes) y rellenar con subarreglos
    emitln(ftext, "    sub sp, sp, #16");
    {
        char mvm[64];
        snprintf(mvm, sizeof(mvm), "    mov w1, #%d", m);
...
{
    if (!(node && node->node_type && strcmp(node->node_type, "Declaracion") == 0))
        return 0;
    ...
    if (decl->dimensiones > 0)
    {
        ...
        if (node->numHijos > 0 && node->hijos[0] && strcmp(node->hijos[0]->node_type ? node->hijos[0]->node_type : "", "ArrayCreation") == 0)
        {
            ...
            // Soporte para arreglos irregulares
            if (decl->dimensiones > dims && dims >= 1)
            {
                ...
                {
                    char stp[96];
                    snprintf(stp, sizeof(stp), "    sub x16, x29, #%d\n    str x0, [x16]", v->offset);
                    emitln(ftext, stp);
                }
                emitln(ftext, "    add sp, sp, #16");
                return 1;
            ...
            // Evaluar tamaños y crear el arreglo
            if (bytes > 0)
            {
                char sub[64];
                ...
                // Evaluar tamaños y almacenarlos en stack
                ...
                if (decl->tipo == STRING || decl->tipo == DOUBLE || decl->tipo == FLOAT)
                    emitln(ftext, "    bl new_array_flat_ptr");
                else
                    emitln(ftext, "    bl new_array_flat");
                char stp[96];
            ...
            {
                char stp[128];
                snprintf(stp, sizeof(stp), "    mov x1, #0\n    sub x16, x29, #%d\n    str x1, [x16]", v->offset);
                emitln(ftext, stp);
            }
        }

        // Soporte para inicializadores
        ...
        {
            // Inicialización de arreglo desde acceso a otro arreglo
            AbstractExpresion *acc = node->hijos[0];

            // Calcular profundidad de índices
            int depth = 0;
            AbstractExpresion *it = acc;
            ...
            if (!rv)
            {
                ...
                if (gi)
                {
                    ...
                    {
                        char stp2[96];
                        snprintf(stp2, sizeof(stp2), "    sub x16, x29, #%d\n    str x0, [x16]", v->offset);
                        emitln(ftext, stp2);
                    }
                    return 1;
                }
                // No local ni global
                char stpn[128];
                snprintf(stpn, sizeof(stpn), "    mov x1, #0\n    sub x16, x29, #%d\n    str x1, [x16]", v->offset);
                emitln(ftext, stpn);
                return 1;
            }
        ...
        // Soporte para ArrayAdd en inicialización
        else if (node->numHijos > 0 && node->hijos[0] && strcmp(node->hijos[0]->node_type ? node->hijos[0]->node_type : "", "ArrayAdd") == 0)
        {
            ...
            emitln(ftext, "    mov w10, #0");
            {
                ...
                char l[64];
                snprintf(l, sizeof(l), "L_copy_decl_%d:", lid);
                emitln(ftext, l);
                emitln(ftext, "    cmp w10, w19");
                char bge[64];
                snprintf(bge, sizeof(bge), "    b.ge L_copy_done_decl_%d", lid);
                emitln(ftext, bge);
                if (base_t == STRING || base_t == DOUBLE || base_t == FLOAT)
                {
                ...
                char blp[64];
                snprintf(blp, sizeof(blp), "    b L_copy_decl_%d", lid);
                emitln(ftext, blp);
                char ldone[64];
                snprintf(ldone, sizeof(ldone), "L_copy_done_decl_%d:", lid);
                emitln(ftext, ldone);
            }
            ...
            if (base_t == STRING)
            {
                if (!emitir_eval_string_ptr(elem_expr, ftext))
                    emitln(ftext, "    mov x1, #0");
        ...
        else if (decl->tipo == STRING)
        {
            if (strcmp(init->node_type, "Primitivo") == 0)
            {
                ...
                if (p->tipo == STRING)
                {
                    ...
                    char l1[64];
                    snprintf(l1, sizeof(l1), "    ldr x1, =%s", lab);
                    emitln(ftext, l1);
                    char st[96];
                    snprintf(st, sizeof(st), "    sub x16, x29, #%d\n    str x1, [x16]", v->offset);
                    emitln(ftext, st);
                ...
                {
                    char st[128];
                    snprintf(st, sizeof(st), "    mov x1, #0\n    sub x16, x29, #%d\n    str x1, [x16]", v->offset);
                    emitln(ftext, st);
                }
            }
            ...
            else if (expresion_es_cadena(init))
            {
                ...
                if (!emitir_eval_string_ptr(init, ftext))
                    emitln(ftext, "    mov x1, #0");
    ...
    else
    {
        ...
        {
            char st[128];
            snprintf(st, sizeof(st), "    mov w1, #0\n    sub x16, x29, #%d\n    str w1, [x16]", v->offset);
            emitln(ftext, st);
        }
    }
    return 1;
