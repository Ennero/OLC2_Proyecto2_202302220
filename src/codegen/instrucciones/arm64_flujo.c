...
void flujo_emit_label(FILE *f, const char *prefix, int id) {
    char lab[64]; snprintf(lab, sizeof(lab), "%s_%d:", prefix, id); flujo_emitln(f, lab);
}

void flujo_break_push(int id) { if (break_label_sp < (int)(sizeof(break_label_stack)/sizeof(break_label_stack[0]))) break_label_stack[break_label_sp++] = id; }
int  flujo_break_peek(void) { return break_label_sp > 0 ? break_label_stack[break_label_sp - 1] : -1; }
void flujo_break_pop(void) { if (break_label_sp > 0) break_label_sp--; }

void flujo_continue_push(int id) { if (continue_label_sp < (int)(sizeof(continue_label_stack)/sizeof(continue_label_stack[0]))) continue_label_stack[continue_label_sp++] = id; }
int  flujo_continue_peek(void) { return continue_label_sp > 0 ? continue_label_stack[continue_label_sp - 1] : -1; }
void flujo_continue_pop(void) { if (continue_label_sp > 0) continue_label_sp--; }
