...
// Emite código ARM64 para imprimir una expresión de cadena
int arm64_emitir_print_stmt(AbstractExpresion *node, FILE *ftext)
{
    // Verificar que el nodo sea una instrucción Print
    if (!(node && node->node_type && strcmp(node->node_type, "Print") == 0))
        return 0;
    ...
    // Iterar sobre cada expresión en la lista de impresión
    for (size_t i = 0; i < (lista ? lista->numHijos : 0); i++)
    {
        ...
        else if (expr->node_type && strcmp(expr->node_type, "Primitivo") == 0)
        {
            ...
            case CHAR:
            {
                ...
                {
                    v = p->valor && p->valor[0] ? (unsigned char)p->valor[0] : 0;
                }
                char line[64];
                snprintf(line, sizeof(line), "    mov w1, #%d", v);
                emitln(ftext, line);
                emitln(ftext, "    bl printf");
                break;
            }
            ...
            default:
            {
                ...
                char line[64];
                snprintf(line, sizeof(line), "    ldr x1, =%s", label);
                emitln(ftext, line);
                emitln(ftext, "    bl printf");
                break;
            }
        ...
        // Manejar expresiones aritméticas y booleanas
        ...
        // Manejar casteos y otros tipos de expresiones
        else if (expr->node_type && strcmp(expr->node_type, "Casteo") == 0)
        {
            ...
            {
                if (ty == DOUBLE || c->tipo_destino == DOUBLE || c->tipo_destino == FLOAT)
                    emitln(ftext, "    ldr x0, =fmt_double");
                else
                    emitln(ftext, "    ldr x0, =fmt_int");
            }
    ...
    {
        char l2[64];
        snprintf(l2, sizeof(l2), "    ldr x1, =%s", nl);
        emitln(ftext, l2);
    }
    emitln(ftext, "    bl printf");
    return 1;
