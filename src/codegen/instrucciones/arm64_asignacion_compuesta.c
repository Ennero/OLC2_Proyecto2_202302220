...
// Helper local para emitir líneas de código
...
// Emite código ARM64 para una asignación compuesta
int arm64_emitir_asignacion_compuesta(AbstractExpresion *node, FILE *ftext)
{
    // Verificar que el nodo sea una asignación compuesta
    if (!(node && node->node_type && strcmp(node->node_type, "AsignacionCompuesta") == 0))
        return 0;
    AsignacionCompuestaExpresion *ac = (AsignacionCompuestaExpresion *)node;
    VarEntry *v = buscar_variable(ac->nombre);

    // Verificar si es una variable global
    if (!v)
    {
        ...
        if (gi->is_const)
        {
            emitln(ftext, "    // asignación compuesta sobre global constante ignorada");
            return 1;
        }
        AbstractExpresion *rhs = node->hijos[0];
        int op = ac->op_type;
        char adr[128];
        snprintf(adr, sizeof(adr), "    ldr x16, =g_%s", ac->nombre);
        emitln(ftext, adr);

        // Emitir operación compuesta según el tipo
    ...
    // Manejo de variable local
    if (v->is_const)
    {
        emitln(ftext, "    // asignación compuesta sobre constante ignorada");
        return 1;
    }
    AbstractExpresion *rhs = node->hijos[0];
    int op = ac->op_type;
    ...
    else if (op == '+' || op == '-' || op == '*' || op == '/' || op == '%')
    {
        ...
        if (op == '+' && v->tipo == STRING)
        {
            ...
            // Cargar puntero actual de la variable 
            if (v->is_ref)
            {
                char lref[128];
                snprintf(lref, sizeof(lref), "    sub x16, x29, #%d\n    ldr x17, [x16]\n    ldr x1, [x17]", v->offset);
                emitln(ftext, lref);
            }
            else
            {
                char l1s[96];
                snprintf(l1s, sizeof(l1s), "    sub x16, x29, #%d\n    ldr x1, [x16]", v->offset);
                emitln(ftext, l1s);
            }
            ...
            {
                char stref[128];
                snprintf(stref, sizeof(stref), "    sub x16, x29, #%d\n    ldr x17, [x16]\n    str x0, [x17]", v->offset);
                emitln(ftext, stref);
            }
            else
            {
                char stl[96];
                snprintf(stl, sizeof(stl), "    sub x16, x29, #%d\n    str x0, [x16]", v->offset);
                emitln(ftext, stl);
            }
            return 1;
        }
