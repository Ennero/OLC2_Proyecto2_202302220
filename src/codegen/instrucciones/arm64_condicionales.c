...
int arm64_emitir_condicional(AbstractExpresion *node, FILE *ftext, EmitirNodoFn gen_node) {
    ...
    // SwitchStatement (idéntico a implementación previa)
    if (strcmp(nt, "SwitchStatement") == 0) {
        ...
        if (is_stringy) {
            ...
            if (!ok) emitir_string_valueof(selector, ftext);
            emitln(ftext, "    mov x19, x1");
        } else {
            TipoDato ty = emitir_eval_numerico(selector, ftext);
            if (ty == DOUBLE) emitln(ftext, "    fcvtzs w19, d0"); else emitln(ftext, "    mov w19, w1");
        }
        int has_default = 0;
        ...
        for (size_t i = 0; i < (case_list ? case_list->numHijos : 0); i++) {
            ...
            if (is_stringy) {
                ...
                if (!emitir_eval_string_ptr(c->hijos[0], ftext)) { emitir_string_valueof(c->hijos[0], ftext); }
