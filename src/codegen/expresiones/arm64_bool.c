...
// Verifica si el nodo produce un resultado booleano
...
// Emite código para evaluar una expresión booleana, dejando el resultado en w1
void emitir_eval_booleano(AbstractExpresion *node, FILE *ftext)
{
    ...
    if (strcmp(t, "Primitivo") == 0)
    {
        ...
        if (p->tipo == BOOLEAN)
        {
            ...
            char line[64];
            snprintf(line, sizeof(line), "    mov w1, #%d", is_true ? 1 : 0);
            emitln(ftext, line);
            return;
        }
        // Otro primitivo: evaluar como numérico y comparar con 0
    ...
    // Identificador: cargar su valor y comparar con 0
    ...
    {
        // Comparación de igualdad de strings vía método equals
        emitln(ftext, "    // EqualsMethod: evaluar punteros de string");
        emitln(ftext, "    // lhs");

        // Evaluar lhs como puntero de string
        ...
        // Evaluar rhs como puntero de string
        ...
        strcmp(t, "MayorIgual") == 0 || strcmp(t, "MenorIgual") == 0)
    {
        ...
        // Si alguno es null, y el otro es string, comparar como puntero
        ...
        if (strcmp(t, "IgualIgual") == 0 || strcmp(t, "Diferente") == 0)
        {
            ...
            if (lhs_is_str && emitir_eval_string_ptr(node->hijos[1], ftext))
            {
                emitln(ftext, "    mov x20, x1");
                rhs_is_str = 1;
            }
        ...
        // Emitir la instrucción de comparación adecuada
        if (strcmp(t, "IgualIgual") == 0)
            emitln(ftext, "    cset w1, eq");
        else if (strcmp(t, "Diferente") == 0)
            emitln(ftext, "    cset w1, ne");
        else if (strcmp(t, "MayorQue") == 0)
            emitln(ftext, "    cset w1, gt");
        else if (strcmp(t, "MenorQue") == 0)
            emitln(ftext, "    cset w1, lt");
        else if (strcmp(t, "MayorIgual") == 0)
            emitln(ftext, "    cset w1, ge");
        else if (strcmp(t, "MenorIgual") == 0)
            emitln(ftext, "    cset w1, le");
        return;
    }
    if (strcmp(t, "And") == 0)
    {
        ...
        emitln(ftext, "    mov w9, w1");
        emitir_eval_booleano(node->hijos[1], ftext);
        emitln(ftext, "    mov w10, w1");
    ...
    // Evaluar como numérico y comparar != 0
    {
        TipoDato ty = emitir_eval_numerico(node, ftext);
        if (ty == DOUBLE)
        {
