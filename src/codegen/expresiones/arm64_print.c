...
// Representa una expresión que se va a concatenar en print/println
static void append_expr_to_tmpbuf(AbstractExpresion *arg, FILE *ftext)
{
    ...
    if (strcmp(t, "Identificador") == 0)
    {
        ...
        if (v && v->tipo == ARRAY)
        {
            ...
            {
                char l1a[128];
                snprintf(l1a, sizeof(l1a), "    sub x16, x29, #%d\n    ldr x1, [x16]\n    ldr x1, [x1]", v->offset);
                emitln(ftext, l1a);
            }
            else
            {
    ...
    {
        int depthb = 0;
        AbstractExpresion *itb = arg;
        while (itb && itb->node_type && strcmp(itb->node_type, "ArrayAccess") == 0)
        {
            depthb++;
            itb = itb->hijos[0];
        }
...
void emitir_string_valueof(AbstractExpresion *arg, FILE *ftext)
{
    ...
    // 1) Si el resultado es booleano, mapear a true/false
    if (nodo_es_resultado_booleano(arg))
    {
        emitir_eval_booleano(arg, ftext);
        emitln(ftext, "    cmp w1, #0");
    ...
    if (strcmp(t, "ArrayAccess") == 0)
    {
        ...
        int depth2 = 0;
        AbstractExpresion *it2 = arg;
        while (it2 && it2->node_type && strcmp(it2->node_type, "ArrayAccess") == 0)
        {
            depth2++;
            it2 = it2->hijos[0];
        }
    ...
    if (expresion_es_cadena(arg))
    {
        ...
        if (!emitir_eval_string_ptr(arg, ftext))
        {
            ...
            char lz[64];
            snprintf(lz, sizeof(lz), "    ldr x1, =%s", null_lab);
            emitln(ftext, lz);
            return;
        }
        ...
        emitln(ftext, "    cmp x1, #0");
        char l2[64];
        snprintf(l2, sizeof(l2), "    ldr x16, =%s", null_lab);
        emitln(ftext, l2);
        emitln(ftext, "    csel x1, x16, x1, eq");
        return;
    }
    ...
    else if (strcmp(t, "ArrayAccess") == 0)
    {
        ...
        int depth3 = 0;
        AbstractExpresion *it3 = arg;
        while (it3 && it3->node_type && strcmp(it3->node_type, "ArrayAccess") == 0)
        {
            depth3++;
            it3 = it3->hijos[0];
        }
        if (it3 && it3->node_type && strcmp(it3->node_type, "Identificador") == 0)
        {
            ...
            if (arm64_array_elem_tipo_for_var(id3->nombre) == CHAR)
                is_char = 1;
        }
    }
    ...
    if (is_char)
    {
        ...
        // Convertir code point (cp) a UTF-8 y duplicar resultado
    ...
    else
    {
        ...
        {
            // Sprintf a buffer temporal en stack con %d para evitar usar tmpbuf
            emitln(ftext, "    sub sp, sp, #128");
            emitln(ftext, "    mov w21, w1");
            emitln(ftext, "    mov x0, sp");
            emitln(ftext, "    ldr x1, =fmt_int");
...
void emitir_imprimir_cadena(AbstractExpresion *node, FILE *ftext)
{
    ...
    if (strcmp(t, "ArrayAccess") == 0)
    {
        ...
        if (it && it->node_type && strcmp(it->node_type, "Identificador") == 0)
        {
            ...
            if (base_t == STRING)
            {
                ...
                emitln(ftext, "    cmp x1, #0");
                char lnull[64];
                snprintf(lnull, sizeof(lnull), "    ldr x16, =%s", null_lab);
                emitln(ftext, lnull);
                emitln(ftext, "    csel x1, x16, x1, eq");
    ...
    if (strcmp(t, "Primitivo") == 0)
    {
        ...
        if (p->tipo == STRING)
        {
            ...
            emitln(ftext, "    ldr x0, =fmt_string");
            char l2[64];
            snprintf(l2, sizeof(l2), "    ldr x1, =%s", lab);
            emitln(ftext, l2);
            emitln(ftext, "    bl printf");
            return;
        }
    ...
    if (strcmp(t, "Casteo") == 0)
    {
        ...
        {
            if (dest == DOUBLE || dest == FLOAT || ty == DOUBLE)
    ...
    if (strcmp(t, "Identificador") == 0)
    {
        ...
        else if (v && v->tipo == ARRAY)
        {
            ...
            emitln(ftext, "    cmp x1, #0");
            char lnull2[64];
            snprintf(lnull2, sizeof(lnull2), "    ldr x16, =%s", null_lab);
            emitln(ftext, lnull2);
            emitln(ftext, "    csel x1, x16, x1, eq");
            emitln(ftext, "    ldr x0, =fmt_string");
            emitln(ftext, "    bl printf");
        ...
        {
            // Si conocemos tipo DOUBLE/STRING/CHAR/BOOLEAN, imprimir acorde; si no, como int
            const GlobalInfo *gi = globals_lookup(id->nombre);
            if (gi)
            {
    ...
    // Resto de expresiones: numérico o booleano
...
// Emitir evaluación de expresión que produce un puntero a cadena en x1
int emitir_eval_string_ptr(AbstractExpresion *node, FILE *ftext)
{
    ...
    if (strcmp(t, "StringJoin") == 0)
    {
        ...
        // Evaluar delimitador a puntero string en x23
        ...
        if (lista && lista->numHijos == 1 && lista->hijos[0] && strcmp(lista->hijos[0]->node_type, "Identificador") == 0)
        {
            ...
            if (bt != NULO && v)
            {
                ...
                if (bt == STRING)
                    emitln(ftext, "    bl join_array_strings");
                else if (bt == INT)
                    emitln(ftext, "    bl join_array_ints");
                else
                {
                ...
                // Duplicar resultado para evitar alias
                emitln(ftext, "    bl strdup");
