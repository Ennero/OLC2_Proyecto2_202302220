...
// Estructura para literales de cadena y números
...
// --- Decodificación de escapes para strings ---
...
// Parsea una secuencia de escape Unicode en formato decimal \uDDDDD
...
// Procesa las secuencias de escape en una cadena de entrada
    ...
    while (i < len)
    {
        if (input[i] == '\\' && i + 1 < len)
        {
            ...
            case 'u':
            {
                ...
                char tmp[4];
                size_t n = utf8_encode_cp(cp, tmp);
                for (size_t k = 0; k < n; k++)
                    out[j++] = tmp[k];
                size_t consumed = 0;

                // contar dígitos consumidos
...
// Escapa una cadena para uso en .asciz
...
// Agrega un literal de cadena y retorna su etiqueta
const char *core_add_string_literal(const char *text)
{
    ...
    // Si ya existe un literal de cadena con el mismo contenido, reutilizar su etiqueta
...
// Agrega un literal de número double y retorna su etiqueta
...
// Emite los literales recolectados al archivo
...
// Resetea los literales recolectados
